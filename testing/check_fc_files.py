        ...
        for lineno, line in enumerate(fd, start=1):
            ...
            if context != NONE_CONTEXT:
                ...
                if not matches:
                    ...
                    if not context_label.startswith('u:object_r:'):
                        print(f"{prefix}SELinux context does not begin with 'u:object_r:' for {path}: {context}")  # noqa
                        retval = False
                    elif not re.match(r'^u:object_r:[0-9A-Za-z_]+$', context_label):
